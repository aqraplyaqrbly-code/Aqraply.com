    ...
    <div className="max-w-7xl mx-auto px-4">
      <div className="flex justify-between items-center h-16">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-bold text-orange-600">Aqraply أقربلي</h1>
        </div>
        <div className="flex items-center gap-4">
...
function StoreImage({
  ...
  const resolvedUrl = useQuery(
    ...
    imageIdOrUrl ? { storageId: imageIdOrUrl } : "skip"
  );
  const src = resolvedUrl || imageIdOrUrl;
  ...
  return (
    <div className={`${className} bg-gray-100 flex items-center justify-center`}>
      <Store className="w-1/3 h-1/3 text-gray-400" />
    </div>
  );
}
...
function ProductDetailModal({ 
  ...
  product: any; 
  onClose: () => void; 
  onAddToCart: (product: any, sizeLabel?: string, color?: string) => void;
  size: string | null;
      ...
      <div className="bg-white w-full sm:max-w-2xl rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="pt-4 space-y-3">
            <button
              onClick={() => onAddToCart(displayProduct, size || undefined, selectedColor || undefined)}
      ...
      <div className="max-w-7xl mx-auto px-4 mb-6">
        <CustomerLocationTracker 
          onLocationUpdate={(location) => {
            // يمكن إضافة منطق لفلترة المتاجر بناءً على الموقع هنا
...
function StoreDetails() {
  ...
  const handleAddToCart = (product: any, sizeLabel?: string, color?: string) => {
    if (!store) return;
        ...
        <>
          ...
          <div className="max-w-7xl mx-auto px-4 -mt-12 relative z-10">
            <div className="bg-white rounded-2xl shadow-2xl p-8 mb-8">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600 pt-4 border-t border-gray-200">
                ...
                <span>{store.location.addressAr}</span>
              </div>
          ...
          {selectedProduct && (
            <ProductDetailModal
              ...
              onAddToCart={(product, sizeLabel, color) => handleAddToCart(product, sizeLabel, color)}
              size={size}
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-8">
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">{profile?.fullName || user?.name || 'مستخدم'}</h2>
