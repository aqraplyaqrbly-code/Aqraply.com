      ...
      <main className="flex-1 h-screen overflow-y-auto">
        ...
        <div className="lg:hidden bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900">لوحة التاجر</h1>
        ...
        {showMobileMenu && (
          <div className="lg:hidden fixed inset-0 z-50 bg-black/50" onClick={() => setShowMobileMenu(false)}>
            <div className="absolute right-0 top-0 w-72 h-full bg-gradient-to-b from-orange-600 to-red-700 text-white" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-orange-500">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-orange-200 truncate max-w-[140px]">
                      {profile.businessNameAr || profile.businessName || profile.fullName}
                    </p>
                  </div>
                </div>
...
function DashboardHome({ profile }: { profile: any }) {
  ...
  const storeOrders = useQuery(
    ...
    storeId ? { storeId: storeId as Id<"stores">, status: undefined } : "skip"
  );
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-b from-gray-50 to-gray-100 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-start text-xs font-semibold text-gray-600 uppercase">الوقت</th>
                <th className="px-6 py-4 text-start text-xs font-semibold text-gray-600 uppercase">إجراء</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {!storeOrders ? (
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        {order.items.slice(0, 2).map((item: any, i: number) => (
                          <div key={i} className="flex items-start gap-2">
                            <span className="font-medium">{item.nameAr} × {item.quantity}</span>
                        ...
                        {order.items.length > 2 && (
                          <div className="text-xs text-gray-400">+{order.items.length - 2} أخرى</div>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-bold text-gray-900">
                    ...
                    <td className="px-6 py-4">
                      {nextStatus[order.status] && (
                        <button
                          ...
                          className="text-xs bg-orange-100 text-orange-700 hover:bg-orange-200 px-3 py-1.5 rounded-lg font-semibold transition-colors whitespace-nowrap"
                        >
