...
import { Crown, Shield, ArrowRight, RefreshCw, Clock, CheckCircle } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(`/admin/reset-password?identifier=${identifier}&type=${identifierType}&otp=${otpValue}`);
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50 flex items-center justify-center p-4" dir="rtl">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900 mb-2">{t('forgotPassword.twoFactorAuth')}</h1>
          <p className="text-gray-500">
            {t('forgotPassword.otpSentTo')} {identifierType === "email" ? t('forgotPassword.yourEmail') : t('forgotPassword.yourPhone')}
          </p>
          <p className="text-sm text-gray-400 mt-2">{identifier}</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              {otp.map((digit, index) => (
                <input
                  ...
                  onPaste={handlePaste}
                  className="w-12 h-14 text-center text-2xl font-bold border-2 border-gray-200 rounded-xl focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all outline-none"
                  required
                />
              ))}
            </div>
            ...
            {canResend ? (
              <button
                ...
                className="w-full py-3 text-purple-600 font-semibold hover:text-purple-700 transition-colors flex items-center justify-center gap-2"
              >
