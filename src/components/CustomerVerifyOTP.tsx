...
import { ShoppingBag, Shield, ArrowRight, RefreshCw, Clock, CheckCircle } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(`/customer/reset-password?identifier=${identifier}&type=${identifierType}&otp=${otpValue}`);
      }
    } catch (error: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              {otp.map((digit, index) => (
                <input
                  ...
                  onPaste={handlePaste}
                  className="w-12 h-14 text-center text-2xl font-bold border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none"
                  required
                />
              ))}
            </div>
            ...
            {canResend ? (
              <button
                ...
                className="w-full py-3 text-blue-600 font-semibold hover:text-blue-700 transition-colors flex items-center justify-center gap-2"
              >
