...
import { Store, Shield, ArrowRight, RefreshCw, Clock, CheckCircle } from "lucide-react";
import { useTranslation } from "react-i18next";

export default function MerchantVerifyOTP() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(`/merchant/reset-password?identifier=${identifier}&type=${identifierType}&otp=${otpValue}`);
      }
    } catch (error: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              {otp.map((digit, index) => (
                <input
                  ...
                  onPaste={handlePaste}
                  className="w-12 h-14 text-center text-2xl font-bold border-2 border-gray-200 rounded-xl focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-all outline-none"
                  required
                />
              ))}
            </div>
            ...
            {canResend ? (
              <button
                ...
                className="w-full py-3 text-orange-600 font-semibold hover:text-orange-700 transition-colors flex items-center justify-center gap-2"
              >
