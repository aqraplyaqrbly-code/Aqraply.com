...
export default function AdminManagement() {
  ...
  const handleSuspend = async (userId: any) => {
    ...
    try {
      ...
      toast.success("تم إيقاف الإداري بنجاح");
    } catch (error: any) {
      toast.error(error.message || "حدث خطأ أثناء إيقاف الإداري");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleActivate = async (userId: any) => {
    ...
    try {
      ...
      toast.success("تم تفعيل الإداري بنجاح");
    } catch (error: any) {
      toast.error(error.message || "حدث خطأ أثناء تفعيل الإداري");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (userId: any) => {
    ...
    try {
      ...
      toast.error(error.message || "حدث خطأ أثناء حذف الإداري");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMakeOwner = async () => {
    if (!confirm("هل أنت متأكد من جعل حسابك owner؟ هذا سيعطيك كل الصلاحيات.")) return;

    setLoading(true);
    try {
      await makeOwner({ sessionToken });
      toast.success("تم جعل حسابك owner بنجاح");
      // Refresh the page to update permissions
      window.location.reload();
    } catch (error: any) {
      toast.error(error.message || "حدث خطأ أثناء جعل الحساب owner");
    } finally {
      setLoading(false);
    }
  };
  ...
  const permissionLabels = {
    ...
    manage_captains: "إدارة الكباتن",
    manage_notifications: "إدارة الإشعارات",
    view_reports: "عرض التقارير",
    manage_settings: "إدارة الإعدادات",
    view_activity_logs: "عرض سجل النشاط",
  };

  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">إدارة الإداريين</h1>
        <p className="text-gray-600">إدارة صلاحيات الإداريين وحساباتهم</p>
      </div>
                  ...
                  <tr key={admin._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        {admin.user?.isOwner && (
                          <span className="text-gray-400 text-xs">لا يمكن تعديل المالك</span>
                        )}
                      </div>
                    </td>
                  </tr>
            ...
            <div className="p-6 border-b">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-gray-900">
                  {isCreating ? "إضافة إداري جديد" : "تعديل صلاحيات الإداري"}
                </h2>
                <button
                  onClick={() => {
                    setEditingAdmin(null);
                  ...
                  >
                    ...
                    {availableUsers.map((user) => (
                      <option key={user._id} value={user._id}>
                        {user.fullName || user.email || "غير معروف"} - {user.email || "بدون بريد إلكتروني"}
                      </option>
                    ))}
                  </select>
                  <p className="mt-1 text-xs text-gray-500">
                    سيتم عرض المستخدمين الذين ليسوا إداريين حالياً
                  </p>
                  {availableUsers.length === 0 && (
                    <p className="mt-2 text-sm text-amber-600">
                      لا يوجد مستخدمين متاحين للتعيين كإداريين
                    </p>
                  )}
                </div>
              )}
